    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            @if (isset($viewMode) && $viewMode)
                ...
                <img src="{{ $article->getFirstMediaUrl('articles') }}" alt="Article Image" class="img-thumbnail"
                    width="300">
                <br>
            ...
            @else
                <form method="POST"
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" id="description" placeholder="Enter Article Description..."
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="content" id="summernote">{{ isset($article) ? $article->content : old('content') }}</textarea>
                        @error('content')
                            <span class="text-danger"><strong>{{ $message }}</strong></span>
                        @enderror
                    </div>
                    ...
                    {{-- <div class="form-group">
                        ...
                        <select class="form-control" name="type" id="type" required>
                            <option value="news"
                                {{ (isset($article) && $article->type == 'news') || old('type') == 'news' ? 'selected' : '' }}>
                                Berita
                            </option>
                            <option value="kindness_story"
                                {{ (isset($article) && $article->type == 'kindness_story') || old('type') == 'kindness_story' ? 'selected' : '' }}>
                                Cerita Kebaikan
                            </option>
                            <option value="release"
                                {{ (isset($article) && $article->type == 'release') || old('type') == 'release' ? 'selected' : '' }}>
                                Rilis
                            </option>
                            <option value="infographics"
                                {{ (isset($article) && $article->type == 'infographics') || old('type') == 'infographics' ? 'selected' : '' }}>
                                Infografis
                            </option>
                        </select>
                        ...
                        <select class="form-control" name="category_id" id="category_id">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ isset($article) && $article->category_id == $category->id
                                        ? 'selected'
                        ...
                        <select class="form-control" name="donation_id" id="donation_id">
                            ...
                            @foreach ($donations as $donation)
                                <option value="{{ $donation->id }}"
                                    {{ isset($selectedDonation) && $selectedDonation->id == $donation->id
                                        ? 'selected'
                                        : (old('donation_id') == $donation->id
                                            ? 'selected'
                                            : '') }}>
                    ...
                    <div class="form-group">
                        ...
                        <div class="mt-3 position-relative" style="display: inline-block;">
                            <img id="image-preview"
                                src="{{ isset($article) ? $article->getFirstMediaUrl('articles') : '#' }}"
                                alt="Image Preview" class="img-thumbnail {{ isset($article) ? '' : 'd-none' }}"
                                width="300">
